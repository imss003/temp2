...
export default function AdminView({ data }) {
  ...
  const deleteUser = async (id) => {
    if (id === 1) return; // Guard clause
    if (window.confirm(`Are you sure you want to remove User #${id}?`)) {
  ...
  const isAutoManagerRole = ["manager", "finance", "audit"].includes(newUser.role);
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      {activeTab === "stats" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <StatCard label="Total Users" val={data.stats.total_users} color="text-blue-600" />
            <StatCard label="Total Requests" val={data.stats.total_requests} color="text-slate-800" />
            <StatCard label="Pending" val={data.stats.pending} color="text-amber-600" />
            <StatCard label="Paid" val={data.stats.paid} color="text-emerald-600" />
          </div>
      ...
      {activeTab === "users" && (
        <div className="bg-white p-6 rounded-2xl border shadow-sm">
          ...
          <form onSubmit={addUser} className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-8">
            <input
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {users.map((u) => {
                ...
                return (
                  <tr key={u.emp_id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase shadow-sm ${
                        u.role === "admin" ? "bg-slate-800 text-white" : "bg-blue-50 text-blue-700"
                      }`}>
                        {u.role}
                      </span>
                    </td>
                    ...
                    <td className="p-4">
                      {manager ? (
                        <div className="leading-tight">
                          <div className="text-sm font-semibold text-gray-700">{manager.name}</div>
                          <div className="text-[10px] text-gray-400">ID: {manager.emp_id}</div>
                        </div>
                      ) : (
                         <span className="text-slate-300 text-xs italic">No Manager</span>
                      )}
                    </td>
      ...
      {activeTab === "policies" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <form onSubmit={savePolicy} className="bg-white p-6 rounded-2xl border shadow-sm space-y-4 h-fit">
            ...
            <input
              ...
              onChange={(e) => setPolicyForm({ ...policyForm, amount_limit: e.target.value })}
            />
          ...
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {policies.length === 0 && (
              <p className="text-slate-400 text-sm italic">No policies defined yet.</p>
            )}
            {policies.map((p) => (
              <div key={p.id} className="bg-white p-5 rounded-2xl border shadow-sm flex justify-between items-center border-l-4 border-l-blue-500">
                <div>
                  <p className="font-black text-slate-800 uppercase text-xs tracking-widest">{p.category}</p>
                  <p className="text-xs text-slate-400 mt-1">Global Limit Applied</p>
                </div>
                <div className="text-right">
                  <p className="text-xl font-black text-blue-600">₹{p.amount_limit}</p>
                  <p className="text-[10px] text-slate-400 uppercase">Per Request</p>
                </div>
              </div>
            ))}
          </div>
