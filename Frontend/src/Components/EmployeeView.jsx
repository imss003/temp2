...
export default function EmployeeView({ requests }) {
  ...
  const [form, setForm] = useState({ category: "", description: "", amount: "", file: null });
        ...
        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {EXPENSE_CATEGORIES.map((cat) => {
                ...
                return (
                  <option key={cat} value={cat}>
                    {cat} {policy ? `(Limit: ₹${policy.amount_limit})` : "(No Limit Set)"}
                  </option>
                );
              })}
            </select>
          ...
          <div className="flex flex-col">
            ...
            {isOverLimit && (
              <p className="text-red-600 text-xs mt-1.5 font-bold flex items-center gap-1 animate-pulse">
                <span>⚠️</span> This exceeds the ₹{activePolicy.amount_limit} category limit.
              </p>
            )}
          </div>
          ...
          {!editingId && (
            <div className="md:col-span-2">
              <label className="text-sm font-semibold text-gray-600 mb-1">Receipt Image</label>
              <input
                type="file"
                accept="image/*"
          ...
          <div className="flex gap-3 md:col-span-2">
            ...
            {editingId && (
              <button 
                type="button"
                onClick={() => { setEditingId(null); setForm({category:"", description:"", amount: "", file:null}); }} 
